...
from .util import partition_num
...
class RandomWalker:
    ...
    def random_walk(self,walk_length, start_vertex, is_edge_sampling=True):
        node_sentence = [start_vertex]
        now_walk_len = 1

        while now_walk_len < walk_length:
            ...
            if len(neighborhood_list) > 0:
                if not is_edge_sampling:
                    ...
                    if len(node_sentence) == 1:
                        next_node_rank = alias_sample(accept=self.alias_nodes[now_node][0],
                                                                        alias=self.alias_nodes[now_node][1])
                        next_node = neighborhood_list[next_node_rank]
                        node_sentence.append(next_node)
                    else:
                        pre_node = node_sentence[-2]
                        edge = (pre_node, now_node)
                        next_node_rank = alias_sample(accept=self.alias_edges[edge][0],
            ...
            for v in nodes:
                if self.p == 1 and self.q == 1:
                    ...
                    walks.append(self.random_walk(
                        walk_length=walk_length, start_vertex=v))
        return walks
    ...
    def preprocess_transition_probs(self):
        ...
        np.save('node_edges',alias_edges)
